...
const LOCAL_KEY = 'black_hole_save';
...
function buildSaveData(state: GameState): SaveData {
  return { version: state.version, timestamp: Date.now(), state };
}
...
export async function loadGame(): Promise<SaveData | null> {
  ...
  const normalizedCloud = cloud && typeof cloud === 'object' && 'state' in cloud ? (cloud as SaveData) : null;
  if (!normalizedCloud) return local;
  if (!local) return normalizedCloud;
  return (normalizedCloud.timestamp || 0) > (local.timestamp || 0) ? normalizedCloud : local;
}
